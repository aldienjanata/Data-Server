...
window.runSeedBanyumas = async function() {
  ...
  try {
    ...
    const newDevices = [
      ...
      { name: 'X86 Server Speedtest', sort_order: 19, total_ports: 14, desc: JSON.stringify(Array.from({length:7},(_,i)=>({ label: `Slot ${i+1}`, ports: [i*2+1, i*2+2] }))), portLabels: Object.fromEntries(Array.from({length:14},(_,i)=>[i+1,`Port ${(i%2)+1}`])) },
